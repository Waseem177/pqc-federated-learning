...
\textbf{Heterogeneous federation (primary evaluation).}
...
model to produce non-discriminative outputs, yielding AUC~0.000.
In this degenerate regime the model assigns lower predicted probability
to positive-class inputs than to negative-class inputs---a complete
inversion of the intended mapping---so our ROC integrator returns
0.000 (a perfectly anti-correlated classifier).
...
Table~\ref{fig:scalability} summarises final AUC and average round
...
\begin{table}[t]
  \caption{PQC-FL-Health Scalability (50 Rounds, 3 Seeds)}
  \label{fig:scalability}
  \centering
  \begin{tabular}{ccc}
    \toprule
    \textbf{Nodes} & \textbf{Final AUC} & \textbf{Avg Round (ms)} \\
    \midrule
    5  & $0.862 \pm 0.010$ & $63.8 \pm 1.0$ \\
    10 & $0.862 \pm 0.010$ & $63.0 \pm 0.2$ \\
    20 & $0.862 \pm 0.010$ & $65.5 \pm 0.8$ \\
    \bottomrule
  \end{tabular}
  \\[3pt]
  {\footnotesize AUC invariant by FedAvg construction (proportional
  partitioning); the key finding is flat PQC latency across all scales.}
\end{table}
...
poisoning attempts (single rogue node, 50$\times$ scale, seed~42).
