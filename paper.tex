...
\author{
  \IEEEauthorblockN{Mohamed Waseem}
...
attacks; a gradient-norm filter catches all gradient poisoning
attempts.
At $\varepsilon = 10$, differential privacy costs only 4.4\% AUC,
while budgets at or below $\varepsilon = 1$ cause severe degradation
due to noise overwhelming the gradient signal in a small model.
All code, datasets, and results are publicly available.
\end{abstract}
...
\begin{itemize}
  \item We design and implement \textbf{PQC-FL-Health}, to our
    knowledge the first federated learning system for healthcare that
    simultaneously provides quantum-safe channel security (ML-KEM-768),
    authenticated updates (ML-DSA-65), Byzantine robustness via
    signature verification and gradient-norm filtering, and
    differential privacy via the Gaussian mechanism.
  ...
  \item We quantify the privacy-utility tradeoff for DP in small-model
    FL, showing that $\varepsilon \ge 10$ costs less than 5\% AUC,
    while $\varepsilon \le 1$ is not viable for a 1531-parameter
    model, providing a practical baseline for future work on
    DP-compatible architectures.
\end{itemize}
...
Our work is the first to combine ML-KEM key encapsulation, ML-DSA
gradient authentication, Byzantine defence-in-depth, and Gaussian DP
in a single healthcare FL system with empirical benchmarks against
RSA-4096.
...
\textbf{Homogeneous federation (primary evaluation).}
...
\begin{table}[t]
  ...
  \begin{tabular}{lccc}
    \toprule
    \textbf{Disease} & \textbf{Local (isolated)} & \textbf{Federated} & $\boldsymbol{\Delta}$ \textbf{AUC} \\
    \midrule
...
delta.
Without the norm filter, the poisoned model reaches AUC~0.567 at
round~20, far below the clean baseline.
With the gradient-norm filter ($3\times$ median threshold), the rogue
node is rejected in \textbf{100\% of rounds (20/20)}, and the
defended model recovers to AUC~0.801.
The delta between defended (0.801) and undefended (0.567) AUC
...
\begin{figure*}[t!]
  ...
  \caption{Byzantine attack evaluation over 20 rounds (node~4 as
    ...
    ML-DSA rejects the rogue node in 100\% of rounds.
    \textbf{Right (Part~B):} Gradient-poisoning attack (50$\times$
    scale). Without defence, final AUC drops to 0.567. With the
    gradient-norm filter, the rogue is rejected in 100\% of rounds
    and AUC recovers to 0.801 (+0.235 delta).}
  \label{fig:byzantine}
\end{figure*}
...
Tighter budgets cause severe degradation: AUC~0.741 at $\varepsilon=5$
and near-random performance at $\varepsilon \le 1$.

This steep degradation at small $\varepsilon$ is a known property
of DP in small-model FL~\cite{geyer2017}: with only 1531 parameters,
the Gaussian noise ($\sigma = 0.484$ at $\varepsilon=1$) overwhelms
the gradient signal ($\|\Delta\|_2 \approx 0.07$).
At $\varepsilon = 0.5$ ($\sigma = 0.969$), the noise-to-signal ratio
exceeds 13$\times$, causing the sigmoid output to saturate and the
model to predict a single class for all inputs, yielding AUC~0.000.
This degenerate case arises because constant predictions make AUC
computation undefined (no threshold separates classes), and we report
0.000 as the numerical result of the undefined AUC integral.
The practical recommendation is that \textbf{$\varepsilon \ge 10$
provides meaningful privacy at acceptable utility cost} for this
model class.
...
\begin{figure*}[t!]
  ...
  \caption{Differential privacy tradeoff
    ...
    \textbf{Left:} Convergence curves per $\varepsilon$ value.
    $\varepsilon=50$ nearly matches the no-DP baseline (dashed).
    \textbf{Right:} Final AUC at round~30 vs.\ privacy budget.
    At $\varepsilon=10$, AUC penalty is only $-4.4\%$.
    At $\varepsilon \le 1$, noise overwhelms the gradient signal.}
  \label{fig:dp}
\end{figure*}
...
\textbf{Model quality is stable across federation sizes.}
Final AUC remains $0.862 \pm 0.010$ at 5, 10, and 20 nodes.
The mixed dataset partitions proportionally as node count grows, so
each node retains a representative data sample and label distribution.
The PQC cryptographic layer is transparent to model quality across
all tested scales.

\textbf{Round latency does not increase with federation size.}
Average round time is 64\,ms at 5 nodes, 63\,ms at 10 nodes,
and 65\,ms at 20 nodes, a variation of under 3\%.
This result reflects the cost structure of ML-KEM and ML-DSA:
...
Quadrupling the federation from 5 to 20 nodes produces no measurable
latency penalty, confirming that PQC overhead scales gracefully
in practice.

\begin{figure*}[t!]
  \centering
  \includegraphics[width=\textwidth]{fig7_scalability.png}
...
attacks, and the gradient-norm filter catches all scaled poisoning
attempts.
The DP layer shows that $\varepsilon \ge 10$ costs under 5\% AUC
for this model class.
Scalability experiments confirm that AUC and round latency remain
stable when the federation grows from 5 to 20 nodes, with PQC
overhead unchanged at 63--65\,ms per round.
...
HSM acceleration, single-rogue-node Byzantine scenarios, and
simulated rather than real network conditions.
Future work should explore multi-task FL architectures (per-disease
